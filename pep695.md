...
change it for existing classes. My initial implementation of hidden function scopes
would instead have looked in the class scope and then in the outer cell variable, so I
had to make a change to the symtable to make hidden functions follow the same rule as
existing classes.
...
name resolution within a hidden function is exactly as if the code was directly within
...
inject names into the scope of the hidden function, because uses of the walrus operator
inside a comprehension bind a name in the enclosing scope. Therefore, I also had to
disallow usages of the walrus operator within comprehensions within hidden functions.
