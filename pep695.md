...
these "hidden functions" and normal functions. Those are described in
[the PEP](https://peps.python.org/pep-0695/#scoping-behavior) and also outlined below.

The implementation of the PEP uses hidden functions in a number of contexts:
...
the hidden function for the class's type parameters), so we use
...
This section shows the disassembly for a few pieces of smaple code using the new syntax
...
# Code executed in the global scope: create the hidden function, call it, and store the result in the name "Alias"
  1           2 PUSH_NULL
...
# Hidden function for executing the type parameters
...
>>> dis.dis("def func[T](a: T, b: int = 1): pass")
  ...
  1           2 PUSH_NULL
            # The defaults for the function, which must be executed outside the hidden function
              4 LOAD_CONST               3 ((1,))
              6 LOAD_CONST               1 (<code object <generic parameters of func> at 0x101628480, file "<dis>", line 1>)
              8 MAKE_FUNCTION            0
             10 SWAP                     2
            # One argument: We pass the defaults tuple as an argument to the hidden function
             12 CALL                     1
...
Disassembly of <code object Cls at 0x1016984f0, file "<dis>", line 1>:
  ...
  1           2 RESUME                   0
            ...
            # This holds (T,), as created in the hidden function for the generic parameters.
...
Therefore, I decided to go back to showing only the alias name in `repr()`:
...
This should usually make type aliases more readable when displayed in places like
documentation, but we should be open to switching the `repr()` to something else on the
...
C struct representing the type. It proved difficult to use a C type and support multiple
inheritance, which is essential for `Generic`, so I switched to a heap type, which works
more like a Python class.
...
- A test turned out to rely on setting a random attribute `blah` on a `TypeVar`,
  apparently to test some behavior related to reference cycles. To accommodate potential
  user code that sets attributes on `TypeVar`\ s, I gave them a `__dict__` with the
...
- At first I used `_Py_union_type_or` to implement `TypeVar.__or__`, the same function
  ...
  strings, while the ones implemented in `typing.py` do. Therefore, I made
  `TypeVar.__or__` delegate to the Python version too, so now using `|` on a `TypeVar`
  still produces a `typing.Union`, not a `types.Union`.
...
- Creating a generic class now calls into several `typing.py` functions implemented in
  ...
  need for calling into Python code.
